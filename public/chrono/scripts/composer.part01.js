...
function onPostActionClick(event) {
        ...
        if (!button) {
          ...
          if (postCard.closest('#threadModalContent')) {
            return;
          }
        ...
        if (action === 'comment') {
          ...
          if (card.closest('#threadModalContent')) {
            var replyTextarea = threadModalContent ? threadModalContent.querySelector('#threadReplyTextarea') : null;
            if (replyTextarea) {
              replyTextarea.focus();
            }
            return;
        ...
        if (typeof ThreadModal !== 'undefined' && ThreadModal && ThreadModal.isOpen &&
            ThreadModal.currentPost && ThreadModal.currentPost.id === postId &&
            typeof ThreadModal.syncMainCardMetrics === 'function') {
          ThreadModal.syncMainCardMetrics();
        }
...
function attachEvents() {
        ...
        if (profileTabPanels) {
          profileTabPanels.addEventListener('click', onPostActionClick);
        }

        if (threadModalContent) {
          threadModalContent.addEventListener('click', onPostActionClick);
        }
