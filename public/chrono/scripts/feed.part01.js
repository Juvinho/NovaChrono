...
function renderPoll(post) {
        ...
        if (typeof PollSystem !== 'undefined' && PollSystem && typeof PollSystem.renderWidget === 'function') {
          return PollSystem.renderWidget(post);
        }
        ...
        var optionsHtml = (poll.options || []).map(function (option) {
          ...
          return (
            '<div class="poll-option">' +
              '<div class="poll-label"><span>' + option.text + '</span><strong>' + pct + '%</strong></div>' +
              '<div class="poll-track"><span class="poll-fill" data-value="' + pct + '"></span></div>' +
            '</div>' +
          );
        }).join('');

        var metaText = 'Encerrada';
        if (!poll.closed) {
          metaText = 'Ativa';
        }
        ...
        return (
          '<section class="poll-card" aria-label="Enquete">' +
            optionsHtml +
            '<div class="poll-meta">' + metaText + ' • ' + (poll.totalVotes || 0) + ' voto' + ((poll.totalVotes || 0) > 1 ? 's' : '') + '</div>' +
          '</section>'
        );
      }
...
function animatePollBars() {
        var bars = Array.prototype.slice.call(document.querySelectorAll('.poll-fill, .poll-bar-fill'));
        bars.forEach(function (bar) {
          var target = Number(bar.getAttribute('data-value'));
          if (!Number.isFinite(target)) {
            target = Number(bar.getAttribute('data-target-width')) || 0;
          }
...
function renderFeed(options) {
        ...
        if (typeof PollSystem !== 'undefined' && PollSystem && typeof PollSystem.refreshAllWidgets === 'function') {
          PollSystem.refreshAllWidgets({ animateResults: false });
        }
      }
