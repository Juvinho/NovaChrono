...
var SETTINGS_ACCENT_FALLBACK = {
  purple: { primary: '#7c5af0', hover: '#9370ff', active: '#5e3fd4', highlight: 'rgba(124,90,240,0.12)', glowLeft: 'rgba(108,60,220,0.18)', glowRight: 'rgba(0,180,190,0.14)' },
  blue: { primary: '#3b82f6', hover: '#60a5fa', active: '#2563eb', highlight: 'rgba(59,130,246,0.12)', glowLeft: 'rgba(30,80,220,0.18)', glowRight: 'rgba(0,150,255,0.14)' },
  teal: { primary: '#00c4cc', hover: '#22d3db', active: '#0099a3', highlight: 'rgba(0,196,204,0.12)', glowLeft: 'rgba(0,160,170,0.18)', glowRight: 'rgba(0,200,180,0.14)' },
  pink: { primary: '#ec4899', hover: '#f472b6', active: '#db2777', highlight: 'rgba(236,72,153,0.12)', glowLeft: 'rgba(200,50,130,0.18)', glowRight: 'rgba(220,80,160,0.14)' },
  orange: { primary: '#f59e0b', hover: '#fbbf24', active: '#d97706', highlight: 'rgba(245,158,11,0.12)', glowLeft: 'rgba(200,100,0,0.18)', glowRight: 'rgba(220,140,0,0.14)' },
  green: { primary: '#22c55e', hover: '#4ade80', active: '#16a34a', highlight: 'rgba(34,197,94,0.12)', glowLeft: 'rgba(20,160,60,0.18)', glowRight: 'rgba(0,200,100,0.14)' }
};

var SETTINGS_ACCENT_ORDER = ['purple', 'blue', 'teal', 'pink', 'orange', 'green'];
var SETTINGS_ACCENT_LABELS = {
  purple: 'Roxo',
  blue: 'Azul',
  teal: 'Teal',
  pink: 'Rosa',
...
function normalizeAccentForSettings(colorId) {
  if (typeof normalizeAccentColorId === 'function') {
    return normalizeAccentColorId(colorId);
  }
  ...
  return SETTINGS_ACCENT_FALLBACK[normalized] ? normalized : 'purple';
}

function getAccentPalette() {
  if (typeof ACCENT_COLORS !== 'undefined' && ACCENT_COLORS && typeof ACCENT_COLORS === 'object') {
    return ACCENT_COLORS;
  }

  return SETTINGS_ACCENT_FALLBACK;
}

function getAccentIds() {
  var palette = getAccentPalette();
  return SETTINGS_ACCENT_ORDER.filter(function (id) {
    return !!palette[id];
  });
}
...
function ensureSettingsState() {
  ...
  AppState.user.accentColor = normalizeAccentForSettings(AppState.user.accentColor || 'purple');

  AppState.settingsPage = AppState.settingsPage || {
    section: 'conta',
    appearance: { accent: AppState.user.accentColor, fontBase: 14, density: AppState.settings.density || 'default' },
    privacy: {
  ...
  AppState.settingsPage.appearance = AppState.settingsPage.appearance || { accent: AppState.user.accentColor, fontBase: 14, density: AppState.settings.density || 'default' };
  AppState.settingsPage.appearance.accent = normalizeAccentForSettings(AppState.settingsPage.appearance.accent || AppState.user.accentColor || 'purple');
  AppState.user.accentColor = AppState.settingsPage.appearance.accent;
...
function setActiveAccentButtons(mount, activeAccent) {
  var normalized = normalizeAccentForSettings(activeAccent);
  Array.prototype.slice.call(mount.querySelectorAll('[data-accent-value]')).forEach(function (btn) {
    btn.classList.toggle('is-active', btn.getAttribute('data-accent-value') === normalized);
  });
}

function spawnAccentRipple(button) {
  if (!button) {
    return;
  }

  var ripple = document.createElement('span');
  ripple.className = 'settings-accent-ripple';
...
var SettingsAppearance = {
  render: function () {
    ...
    var currentAccent = normalizeAccentForSettings(settings.appearance.accent);
    var accentPalette = getAccentPalette();
    ...
    var accents = getAccentIds().map(function (name) {
      var color = accentPalette[name] ? accentPalette[name].primary : accentPalette.purple.primary;
      var label = SETTINGS_ACCENT_LABELS[name] || name;
      return '<button type="button" class="settings-accent-dot ' + (name === currentAccent ? 'is-active' : '') + '" data-accent-value="' + name + '" aria-label="Selecionar cor ' + escapeHtml(label) + '" title="' + escapeHtml(label) + '" style="--accent-preview:' + color + '"></button>';
    }).join('');
...
var SettingsPage = {
  ...
  bindSection: function (section, mount) {
    ...
    if (section === 'aparencia') {
      ...
      var accentState = ensureSettingsState();
      var currentAccent = normalizeAccentForSettings((accentState.appearance && accentState.appearance.accent) || (AppState.user && AppState.user.accentColor) || 'purple');

      if (typeof ThemeController !== 'undefined' && ThemeController && typeof ThemeController.applyAccentColor === 'function') {
        ThemeController.applyAccentColor(currentAccent, false, {
          persist: false,
          pulse: false,
          forceCurrent: true
        });
      ...
      setActiveAccentButtons(mount, currentAccent);

      Array.prototype.slice.call(mount.querySelectorAll('[data-accent-value]')).forEach(function (btn) {
        var accent = normalizeAccentForSettings(btn.getAttribute('data-accent-value') || 'purple');

        btn.addEventListener('mouseenter', function () {
          if (typeof ThemeController !== 'undefined' && ThemeController && typeof ThemeController.applyAccentColor === 'function') {
            ThemeController.applyAccentColor(accent, true);
          }
        });
        ...
        btn.addEventListener('mouseleave', function () {
          if (typeof ThemeController !== 'undefined' && ThemeController && typeof ThemeController.restoreCurrent === 'function') {
            ThemeController.restoreCurrent();
          }
        });
        ...
        btn.addEventListener('click', function () {
          var state = ensureSettingsState();
          state.appearance.accent = accent;
          AppState.user = AppState.user || {};
          AppState.user.accentColor = accent;

          if (typeof ThemeController !== 'undefined' && ThemeController && typeof ThemeController.applyAccentColor === 'function') {
            ThemeController.applyAccentColor(accent, false);
          } else {
            var palette = getAccentPalette();
            var preset = palette[accent] || palette.purple;
            ...
            document.documentElement.style.setProperty('--color-primary-active', preset.active);
            document.documentElement.style.setProperty('--color-primary-highlight', preset.highlight);
            document.documentElement.style.setProperty('--glow-left', preset.glowLeft);
            document.documentElement.style.setProperty('--glow-right', preset.glowRight);
          }

          setActiveAccentButtons(mount, accent);
          spawnAccentRipple(btn);
        });
      });
