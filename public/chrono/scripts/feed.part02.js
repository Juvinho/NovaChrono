...
var FeedFilters = {
  ...
  dateFilterKey: '',
        ...
        emptyConfig: {
          ...
          date: {
            icon: 'calendar-days',
            title: 'Nenhum post nesta data',
            text: 'Tente outra data na timeline ou limpe o filtro para ver todo o feed.'
          }
        },
        ...
        init: function () {
          ...
          if (typeof FeedRenderer !== 'undefined' && FeedRenderer && typeof FeedRenderer.flushPendingDateFilter === 'function') {
            FeedRenderer.flushPendingDateFilter();
          }
        },
        ...
        getPostDateKey: function (postEl) {
          var createdAt = Number(postEl && postEl.getAttribute('data-created-at'));
        ...
        matchesDate: function (postEl) {
          if (!this.dateFilterKey) {
            return true;
          }

          return this.getPostDateKey(postEl) === this.dateFilterKey;
        },

        shouldShowPost: function (postEl, filter) {
          return this.matchesFilter(postEl, filter) && this.matchesSearch(postEl) && this.matchesDate(postEl);
        },

        setDateFilter: function (dateKey, animated) {
          this.dateFilterKey = String(dateKey || '').trim();

          if (!this.isInitialized) {
            return;
          }

          this.applyFilter(this.current, animated !== false);
        },
        ...
        hidePostImmediately: function (postEl) {
          ...
          postEl.classList.remove('date-match');
        },
        ...
        applyFilter: function (filter, animated) {
          ...
          function commitVisibility() {
            ...
            self.posts.forEach(function (postEl) {
              ...
              postEl.classList.toggle('date-match', !!self.dateFilterKey && self.matchesDate(postEl) && visible);
        ...
        updateEmptyState: function (visibleCount) {
          ...
          this.showEmptyState(this.dateFilterKey ? 'date' : this.current);
        },
        ...
        syncCurrentView: function (postEl) {
          ...
          postEl.classList.toggle('search-match', !!state.query && visible);
          postEl.classList.toggle('date-match', !!this.dateFilterKey && this.matchesDate(postEl) && visible);
