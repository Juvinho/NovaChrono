...
renderMetricsHtml: function () {
          ...
          var counts = (postId && typeof VoteSystem !== 'undefined') ? VoteSystem.getCounts(postId) : { up: 0, stable: 0, down: 0 };
          ...
          return (
            '<section class="thread-metrics-bar">' +
              ...
              '<span class="thread-metric"><span class="metric-value thread-vote-stables" style="color:#7c5af0">' + counts.stable + '</span><span class="metric-label">Estavel</span></span>' +
...
renderActionsHtml: function () {
          ...
          var postId = this.currentPost ? this.currentPost.id : '';
          var voteState = (postId && typeof VoteSystem !== 'undefined') ? VoteSystem.getState(postId) : null;
          var currentVote = voteState ? voteState.userVote : null;
          var score = (postId && typeof VoteSystem !== 'undefined') ? VoteSystem.getScore(postId) : 0;
          var scoreStr = score > 0 ? '+' + score : String(score);
          ...
          return (
            '<section class="thread-actions-bar">' +
              ...
              '<div class="vote-cluster vote-cluster-thread" data-vote-cluster="true" data-post-id="' + postId + '">' +
                '<button class="vote-quick vote-up' + (currentVote === 'up' ? ' is-active' : '') + '" data-vote-quick="up" type="button" aria-label="Upvote"><i data-lucide="chevron-up"></i></button>' +
                '<span class="vote-score-value ' + scoreClass + '" data-vote-score="true">' + scoreStr + '</span>' +
                '<button class="vote-quick vote-stable' + (currentVote === 'stable' ? ' is-active' : '') + '" data-vote-quick="stable" type="button" aria-label="Estavel"><i data-lucide="minus"></i></button>' +
                '<button class="vote-quick vote-down' + (currentVote === 'down' ? ' is-active' : '') + '" data-vote-quick="down" type="button" aria-label="Downvote"><i data-lucide="chevron-down"></i></button>' +
              '</div>' +
